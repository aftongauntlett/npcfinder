-- Fix kanban singleton: Add default sections to ensure_singleton_board and merge multiple boards safely
-- This migration addresses issues from the initial singleton migration
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- Tasks are already linked to the section via section_id, so they follow automatically
...
-- Step 2: Update ensure_singleton_board function to auto-create default sections for kanban
...
COMMENT ON FUNCTION ensure_singleton_board IS 'Ensures a singleton board exists for global collection types (job_tracker, recipe, kanban). Automatically creates default sections for new kanban boards. Called explicitly by application code. Uses search_path = public for security.';

-- Step 3: Add default sections to existing kanban boards that don't have any
DO $$
DECLARE
...
BEGIN
  ...
  LOOP
    ...
    IF v_section_count = 0 THEN
      INSERT INTO task_board_sections (board_id, name, display_order)
      VALUES 
        (v_board_record.id, 'To Do', 0),
        (v_board_record.id, 'In Progress', 1),
        (v_board_record.id, 'Done', 2);
      
      RAISE NOTICE 'Created default sections for existing kanban board %', v_board_record.id;
    END IF;
  END LOOP;
END $$;

-- Migration: Add icon field to media lists
-- Date: 2025-12-13
-- Purpose: Allow per-list icon selection in Media Lists UI.

ALTER TABLE public.media_lists
  ADD COLUMN IF NOT EXISTS icon text NULL;

COMMENT ON COLUMN public.media_lists.icon IS 'Optional icon identifier for the media list (curated per-domain icon set).';

-- Update view to include icon

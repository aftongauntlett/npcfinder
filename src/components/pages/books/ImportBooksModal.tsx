...
} from "../../../utils/batchBookSearch";
import { useBatchAddToReadingList } from "../../../hooks/useReadingListQueries";
...
export default function ImportBooksModal({
  ...
  const batchAddMutation = useBatchAddToReadingList();

  // Parse and search
  const handleSearch = async () => {
    if (!pastedText.trim()) {
      setParseErrors(["Please enter or upload a list of books"]);
      return;
    }
    ...
    if (parseResult.titles.length === 0) {
      setParseErrors(["No valid book titles found"]);
      return;
    }
    ...
    try {
      const results = await batchSearchBooks(parseResult.titles, {
          ...
          <div className="space-y-6">
            ...
            <div>
              <textarea
                ...
                placeholder="Paste your list here... (one book per line, comma-separated, or JSON format)"
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              <div className="space-y-1">
                <p>• One book per line</p>
                <p>• Comma-separated: Book 1, Book 2, Book 3</p>
                <p>• JSON: ["Book 1", "Book 2"]</p>
              </div>
            </div>
          ...
          <div className="space-y-6">
                    ...
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                        <input
                          ...
                          checked={
                            selectedResults.size ===
                            searchResults.filter((r) => r.book).length
                          }
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedResults(
                        ...
                        <td className="px-4 py-3 text-sm">
                          {result.book ? (
                            <div className="flex items-center space-x-2">
                              {result.book.poster_url && (
                                <img
                                  src={result.book.poster_url}
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              >
                ...
                {selectedResults.size === 1 ? "Book" : "Books"}
              </Button>
            </div>
          </div>
        )}
